...
/** Roles for which TOTP is mandatory ({@code tpub.security.totp.required-roles}). */
@Slf4j
@Component
public class TotpPolicy {
    ...
    public TotpPolicy(TpubProperties properties) {
        ...
        for (String raw : values) {
            ...
            for (String part : raw.split(",")) {
                ...
                try {
                    ...
                    log.warn("tpub.security.totp.required-roles : rôle inconnu « {} » ignoré.", name);
                    continue;
                }
                if (role == RoleCode.ANNONCEUR) {
                    log.warn("tpub.security.totp.required-roles : ANNONCEUR ne peut pas être obligatoire, ignoré.");
                    continue;
                }
                roles.add(role);
            }
