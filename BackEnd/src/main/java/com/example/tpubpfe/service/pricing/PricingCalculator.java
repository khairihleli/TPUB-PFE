...
import com.example.tpubpfe.service.AvailabilityRules;
...
public final class PricingCalculator {
    ...
    public static List<Band> bands(GeoPricingProperties.Dynamic config) {
        ...
        if (raw == null || raw.isEmpty()) {
            throw new IllegalStateException("tpub.pricing.dynamic.hour-bands : au moins une tranche horaire est requise.");
        }
        ...
        for (GeoPricingProperties.Dynamic.HourBand band : raw) {
            ...
            if (end <= start || band.getMultiplier() == null || band.getMultiplier().signum() <= 0) {
                throw new IllegalStateException("tpub.pricing.dynamic.hour-bands : tranche invalide " + band.getStart()
                        + "–" + band.getEnd() + ".");
            }
        ...
        for (Band band : bands) {
            if (band.startSecond() != cursor) {
                throw new IllegalStateException(
                        "tpub.pricing.dynamic.hour-bands : les tranches doivent couvrir 00:00–24:00 sans trou ni chevauchement.");
            }
    ...
    static int parseSecond(String hhmm) {
        if (hhmm == null || !hhmm.trim().matches("\\d{1,2}:\\d{2}")) {
            throw new IllegalStateException("tpub.pricing.dynamic.hour-bands : heure invalide « " + hhmm + " » (HH:mm).");
        }
        String[] parts = hhmm.trim().split(":");
        int hours = Integer.parseInt(parts[0]);
        int minutes = Integer.parseInt(parts[1]);
        if (minutes > 59 || hours > 24 || (hours == 24 && minutes != 0)) {
            throw new IllegalStateException("tpub.pricing.dynamic.hour-bands : heure invalide « " + hhmm + " ».");
        }
