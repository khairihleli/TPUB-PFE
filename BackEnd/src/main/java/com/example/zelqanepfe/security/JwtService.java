...
public class JwtService {
    ...
    public JwtService(ZelqaneProperties zelqaneProperties, Clock clock) {
        this.zelqaneProperties = zelqaneProperties;
        this.clock = clock;
    }

    /** Token lifetime configured by {@code zelqane.jwt.expiration-ms}. */
    public long expirationMs() {
        return zelqaneProperties.getJwt().getExpirationMs();
    }
    ...
    private SecretKey getSigningKey() {
        byte[] keyBytes = zelqaneProperties.getJwt().getSecret().getBytes(StandardCharsets.UTF_8);
