...
Admin (ADMINISTRATEUR, never on self → 400 `ROLE_NOT_ALLOWED`): `POST /api/admin/users/{id}/2fa/reset` → `AdminUserResponse` (TOTP off, recovery codes deleted, sessions revoked; audit `USER_2FA_RESET`) · `POST /api/admin/users/{id}/require-password-change` → `AdminUserResponse` (flag set, sessions revoked `REVOKED_BY_ADMIN`; audit `USER_PASSWORD_CHANGE_REQUIRED`) · `POST /api/admin/users/{id}/password/reset` → `TemporaryPasswordResponse { userId, email, temporaryPassword, mustChangePassword: true, revokedSessions }`: the password is replaced by a generated one **returned once** (20 characters, never stored in clear), `mustChangePassword` is set and every session is revoked; audit `USER_PASSWORD_RESET`. The UI shows the temporary password once in the account dialog (« Réinitialiser le mot de passe »), and `scripts/seed-demo.mjs` uses it to realign demo accounts created before round 2.
