# ZELQANE — Plan UX exécutable (espace annonceur & back-office)
...
> Binding references: `SPEC.md`, `COMPONENTS.md`, `api-contract.md` (§7 backend limits), `zelqane-brief.md` (no invented figures, honest labels), `NETWORK-MAP-SPEC.md`, `design-reference.md` §7.3.
...
5. Help menu « ? » (radix DropdownMenu): « Raccourcis clavier » (opens ShortcutsDialog), « Comment ça marche » (`/fonctionnement`, new tab), « FAQ » (`/faq`, new tab), « Contacter ZELQANE » (mailto), « Voir le site ZELQANE » (new tab, `rel="noopener"`).
6. Contextual create CTA (espace only, rule in §2).

**Account menu (sidebar bottom on desktop; drawer bottom on mobile):** Profil (espace only), « Aide & raccourcis », « Voir le site ZELQANE » (new tab), separator, « Se déconnecter ». Profil is removed from `ESPACE_NAV`.
...
- `document.title` follows the last trail label through each route's metadata. Client pages that learn an object name call `useDocumentTitle(name)` (« Ouverture boutique La Marsa — Campagnes — ZELQANE »).
...
| ANNONCEUR opens `/admin/**` (or staff opens `/espace/**`) | Middleware and layouts redirect to the role home with `?acces=reserve`. The shell shows an info toast once, « Cette section est réservée à l'équipe ZELQANE. » (staff: « Cette section est réservée aux comptes annonceurs. »), then strips the param with `replaceState`. |
...
### 3.6 Glossaire (`src/content/glossary.ts`, documented in `zelqane-brief.md`)
...
Reservation status labels (advertiser): `TEMPORAIRE` « Bloqué · en attente de décision ZELQANE » (short: « Bloqué »), `CONFIRMEE` « Confirmé », `ANNULEE` « Libéré », `EXPIREE` « Passé ».
...
| Slow / timeout | as Error, Clock/Hourglass icon | « Le service met trop de temps à répondre » / « ZELQANE met plus de temps que prévu à répondre. Nouvel essai… » | — |
...
| `status-pending` | waiting for ZELQANE | amber (`--warning`) |
...
| APPROVED_BY_AI | En examen ZELQANE | « Analyse favorable » | pending |
| REVIEW_REQUIRED | En examen ZELQANE | « Quelques points à vérifier par l'équipe » | pending |
...
| BLOCKED | Refusée | « Contactez ZELQANE pour connaître le motif » | problem |
...
2. **Waiting on ZELQANE:** « Examen par l'équipe ZELQANE en jours ouvrés (lun–ven, 9 h–18 h). » This uses the support hours already published on the profile page. No duration promise.
...
4. **Stepper truth:** `VALIDATED`/`SCHEDULED` show « Validation ZELQANE ✓ » and a Diffusion step labelled « Dans 5 jours », not current. ACTIVE in window marks Diffusion current with pulse.
5. **« Prochaine étape » card** (A2) on validated campaigns: « Envoyez votre visuel à ZELQANE avant le {startDate} » + mailto prefilled with `CAMP-000xx`, name and period (upload doesn't exist). On BLOCKED: « Contactez ZELQANE pour connaître le motif » + mailto. On REJECTED_BY_AI: « Dupliquer et corriger ».
6. **Reservations:** « Bloqué · en attente de décision ZELQANE » with tooltip « Le créneau est retenu pour cette campagne jusqu'à la décision de ZELQANE. Il n'est pas libérable en ligne. »
...
8. **Proof of broadcast:** the auth panel and Statistiques state the real situation: « Journal de diffusion par campagne — mise en service progressive. Les diffusions sont journalisées côté ZELQANE ; le rapport annonceur n'est pas encore ouvert. » + contact link.
...
`useFormDraft<T>({ key, value, dirty, userId, version })` saves to sessionStorage (`zelqane:draft:v{version}:{userId}:{key}`) debounced 500 ms while dirty. It returns `{ restoredValue, restoredAt, discard(), clear() }`. On mount with a stored value, the form shows `<DraftRestoreNotice>` (info Alert): « Saisie restaurée ({relative time}). » + « Effacer ». It is cleared on successful save/submit and ignored if older than 24 h. All access is wrapped in try/catch; if storage is unavailable, the notice never shows.
...
`COMPONENTS.md` gains sections: page anatomy, button ladder, states recipes, status palette, date fields, form safety, URL state, palette/shortcuts registration. `zelqane-brief.md` gains the glossary.
...
3. **Navigation groups, account menu, help menu, badges** (IA-18, IA-20, IA-21, IA-13 nav label). AC: Profil is only in the account menu. « Voir le site ZELQANE » opens in a new tab. With `demo@annonceur.tn`, Campagnes shows the draft count badge when > 0. With `admin@zelqane.local`, Modération shows the same count as the overview hero. Badges hidden at 0.
...
11. **Status palette + vocabulary + filters** (VD-01, VD-02 lib part, FLOW-19 lib part, VD-06 labels lib). AC: `campaign-status.test.ts` asserts no two lifecycle phases share a tone, every bucket tone equals its pill tone, annonceur labels for APPROVED_BY_AI/REVIEW_REQUIRED are both « En examen ZELQANE », legacy `?statut=` values map. The contrast test passes.
...
20. **Glossary + docs** (IA-15). AC: `src/content/glossary.ts` exists; COMPONENTS.md and zelqane-brief.md updated.
...
1. **First-run dashboard** (FLOW-10, VD-11, IA-22, IA-13 onboarding copy). With 0 campaigns (`client@zelqane.com`): hero « Lancez votre première campagne » with primary « Créer ma première campagne » and secondary « Voir les Porteurs en 3D » (→ `/espace/reseau`); 3 milestones from real data only (Brouillon créé · Porteurs réservés · Soumise), progress « 0 sur 3 »; « Comment ça marche » 3 honest steps (Brouillon → analyse IA puis examen par l'équipe ZELQANE → diffusion sur la période validée) replacing the photo; non-counted link « Vérifier les informations de votre société » (→ profil). KPI tiles and Estimations card hidden until ≥ 1 campaign. « À faire » shows the next milestone instead of « Rien à faire ». Profile page copy: « Ce sont les informations transmises à ZELQANE pour examiner vos campagnes. Une correction ? Contactez votre interlocuteur. ». AC: RTL with empty `/mine`: no KPI tile, exactly one primary button, no « Rien à faire pour le moment ».
